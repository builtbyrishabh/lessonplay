...
function sandboxSection(ctx: LessonPromptContext): string[] {
  return [
    ...
    "The game-authoring skills listed below are the contract the engine expects, not background reading. Load the relevant one with `skill` BEFORE writing any game code.",
    "",
    "Paths:",
    `- ${GAME_ROOT} — this chat's game project, and where commands run by default. Ordinary disk: fast to build and test in, but the sandbox can be reclaimed between messages, so nothing here is safe until you publish.`,
    `- ${ENGINE_ROOT} — @learn-loop/core and the game templates, dependencies already installed. Read from it; copy templates out of it; do not edit it in place.`,
    "",
    "Which tool: `read` a specific file you know the path of; `bash` to look around (ls, grep, find) or to run anything (npm install/test/build, cp, mkdir); `write` for new or wholesale-replaced files — never author code through a bash heredoc, the escaping goes wrong; `edit` for a small change to an existing file.",
    "",
    "Checking the game:",
    ...
    "",
    "Explore before you edit: read the skill, look at the template, then write files. Run `validate` and the engine's tests/build before telling the teacher something works, and report failures honestly instead of guessing.",
  ];
}
